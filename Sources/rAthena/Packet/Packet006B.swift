...
extension Packet006B {
    ...
    public struct CharInfo: BinaryDecodable, BinaryEncodable {
        ...
        public static var size: UInt16 {
            ...
            size += 4
            if CONFIG_PACKETVER >= 20141022 {
                size += 2   // body
            }
            size += 2
            if (CONFIG_PACKETVER >= 20100720 && CONFIG_PACKETVER <= 20100727) || CONFIG_PACKETVER >= 20100803 {
                size += 16  // last map
            }
            if CONFIG_PACKETVER >= 20100803 {
                size += 4   // delete date
            }
            if CONFIG_PACKETVER >= 20110111 {
                size += 4   // robe
            }
            if CONFIG_PACKETVER != 20111116 {
                if CONFIG_PACKETVER >= 20110928 {
                    size += 4   // char moves
                }
                if CONFIG_PACKETVER >= 20111025 {
                    size += 4   // rename enabled
                }
                if CONFIG_PACKETVER >= 20141016 {
                    size += 1   // sex
                }
            }
            return size
