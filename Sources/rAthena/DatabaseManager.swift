...
public class DatabaseManager {
    ...
    private func create_cart_inventory(_ db: Connection) throws {
        let cart_inventory = Table("cart_inventory")
        let id = Expression<Int64>("id")
        let char_id = Expression<Int64>("char_id")
        let nameid = Expression<Int64>("nameid")
        let amount = Expression<Int64>("amount")
        let equip = Expression<Int64>("equip")
        let identify = Expression<Int64>("identify")
        let refine = Expression<Int64>("refine")
        let attribute = Expression<Int64>("attribute")
        let card0 = Expression<Int64>("card0")
        let card1 = Expression<Int64>("card1")
        ...
        let option_val4 = Expression<Int64>("option_val4")
        let option_parm4 = Expression<Int64>("option_parm4")
        let expire_time = Expression<Int64>("expire_time")
        let bound = Expression<Int64>("bound")
        let unique_id = Expression<Int64>("unique_id")
        let enchantgrade = Expression<Int64>("enchantgrade")

        try db.run(cart_inventory.create(ifNotExists: true) { t in
            t.column(id, primaryKey: .autoincrement)
            t.column(char_id, defaultValue: 0)
            t.column(nameid, defaultValue: 0)
            t.column(amount, defaultValue: 0)
            t.column(equip, defaultValue: 0)
            t.column(identify, defaultValue: 0)
            t.column(refine, defaultValue: 0)
            t.column(attribute, defaultValue: 0)
            t.column(card0, defaultValue: 0)
            t.column(card1, defaultValue: 0)
            ...
            t.column(option_val4, defaultValue: 0)
            t.column(option_parm4, defaultValue: 0)
            t.column(expire_time, defaultValue: 0)
            t.column(bound, defaultValue: 0)
            t.column(unique_id, defaultValue: 0)
            t.column(enchantgrade, defaultValue: 0)
        })

        try db.run(cart_inventory.createIndex(char_id, ifNotExists: true))
    }

    private func create_char(_ db: Connection) throws {
        let char = Table("char")
        let char_id = Expression<Int64>("char_id")
        let account_id = Expression<Int64>("account_id")
        let char_num = Expression<Int64>("char_num")
        let name = Expression<String>("name")
        let `class` = Expression<Int64>("class")
        let base_level = Expression<Int64>("base_level")
        let job_level = Expression<Int64>("job_level")
        let base_exp = Expression<Int64>("base_exp")
        let job_exp = Expression<Int64>("job_exp")
        let zeny = Expression<Int64>("zeny")
        let str = Expression<Int64>("str")
        let agi = Expression<Int64>("agi")
        let vit = Expression<Int64>("vit")
        let int = Expression<Int64>("int")
        let dex = Expression<Int64>("dex")
        let luk = Expression<Int64>("luk")
        let pow = Expression<Int64>("pow")
        let sta = Expression<Int64>("sta")
        let wis = Expression<Int64>("wis")
        let spl = Expression<Int64>("spl")
        let con = Expression<Int64>("con")
        let crt = Expression<Int64>("crt")
        let max_hp = Expression<Int64>("max_hp")
        let hp = Expression<Int64>("hp")
        let max_sp = Expression<Int64>("max_sp")
        let sp = Expression<Int64>("sp")
        let max_ap = Expression<Int64>("max_ap")
        let ap = Expression<Int64>("ap")
        let status_point = Expression<Int64>("status_point")
        let skill_point = Expression<Int64>("skill_point")
        let trait_point = Expression<Int64>("trait_point")
        let option = Expression<Int64>("option")
        let karma = Expression<Int64>("karma")
        let manner = Expression<Int64>("manner")
        let party_id = Expression<Int64>("party_id")
        let guild_id = Expression<Int64>("guild_id")
        let pet_id = Expression<Int64>("pet_id")
        let homun_id = Expression<Int64>("homun_id")
        let elemental_id = Expression<Int64>("elemental_id")
        let hair = Expression<Int64>("hair")
        let hair_color = Expression<Int64>("hair_color")
        let clothes_color = Expression<Int64>("clothes_color")
        let body = Expression<Int64>("body")
        let weapon = Expression<Int64>("weapon")
        let shield = Expression<Int64>("shield")
        let head_top = Expression<Int64>("head_top")
        let head_mid = Expression<Int64>("head_mid")
        let head_bottom = Expression<Int64>("head_bottom")
        let robe = Expression<Int64>("robe")
        let last_map = Expression<String>("last_map")
        let last_x = Expression<Int64>("last_x")
        let last_y = Expression<Int64>("last_y")
        let save_map = Expression<String>("save_map")
        let save_x = Expression<Int64>("save_x")
        let save_y = Expression<Int64>("save_y")
        let partner_id = Expression<Int64>("partner_id")
        let online = Expression<Int64>("online")
        let father = Expression<Int64>("father")
        let mother = Expression<Int64>("mother")
        let child = Expression<Int64>("child")
        let fame = Expression<Int64>("fame")
        let rename = Expression<Int64>("rename")
        let delete_date = Expression<Int64>("delete_date")
        let moves = Expression<Int64>("moves")
        let unban_time = Expression<Int64>("unban_time")
        let font = Expression<Int64>("font")
        let uniqueitem_counter = Expression<Int64>("uniqueitem_counter")
        let sex = Expression<String>("sex")
        let hotkey_rowshift = Expression<Int64>("hotkey_rowshift")
        let hotkey_rowshift2 = Expression<Int64>("hotkey_rowshift2")
        let clan_id = Expression<Int64>("clan_id")
        let last_login = Expression<String?>("last_login")
        let title_id = Expression<Int64>("title_id")
        let show_equip = Expression<Int64>("show_equip")

        try db.run(char.create(ifNotExists: true) { t in
            t.column(char_id, primaryKey: .autoincrement)
            t.column(account_id, defaultValue: 0)
            t.column(char_num, defaultValue: 0)
            t.column(name, defaultValue: "")
            t.column(`class`, defaultValue: 0)
            t.column(base_level, defaultValue: 1)
            t.column(job_level, defaultValue: 1)
            t.column(base_exp, defaultValue: 0)
            t.column(job_exp, defaultValue: 0)
            t.column(zeny, defaultValue: 0)
            t.column(str, defaultValue: 0)
            t.column(agi, defaultValue: 0)
            t.column(vit, defaultValue: 0)
            t.column(int, defaultValue: 0)
            t.column(dex, defaultValue: 0)
            t.column(luk, defaultValue: 0)
            t.column(pow, defaultValue: 0)
            t.column(sta, defaultValue: 0)
            t.column(wis, defaultValue: 0)
            t.column(spl, defaultValue: 0)
            t.column(con, defaultValue: 0)
            t.column(crt, defaultValue: 0)
            t.column(max_hp, defaultValue: 0)
            t.column(hp, defaultValue: 0)
            t.column(max_sp, defaultValue: 0)
            t.column(sp, defaultValue: 0)
            t.column(max_ap, defaultValue: 0)
            t.column(ap, defaultValue: 0)
            t.column(status_point, defaultValue: 0)
            t.column(skill_point, defaultValue: 0)
            t.column(trait_point, defaultValue: 0)
            t.column(option, defaultValue: 0)
            t.column(karma, defaultValue: 0)
            t.column(manner, defaultValue: 0)
            t.column(party_id, defaultValue: 0)
            t.column(guild_id, defaultValue: 0)
            t.column(pet_id, defaultValue: 0)
            t.column(homun_id, defaultValue: 0)
            t.column(elemental_id, defaultValue: 0)
            t.column(hair, defaultValue: 0)
            t.column(hair_color, defaultValue: 0)
            t.column(clothes_color, defaultValue: 0)
            t.column(body, defaultValue: 0)
            t.column(weapon, defaultValue: 0)
            t.column(shield, defaultValue: 0)
            t.column(head_top, defaultValue: 0)
            t.column(head_mid, defaultValue: 0)
            t.column(head_bottom, defaultValue: 0)
            t.column(robe, defaultValue: 0)
            t.column(last_map, defaultValue: "")
            t.column(last_x, defaultValue: 53)
            t.column(last_y, defaultValue: 111)
            t.column(save_map, defaultValue: "")
            t.column(save_x, defaultValue: 53)
            t.column(save_y, defaultValue: 111)
            t.column(partner_id, defaultValue: 0)
            t.column(online, defaultValue: 0)
            t.column(father, defaultValue: 0)
            t.column(mother, defaultValue: 0)
            t.column(child, defaultValue: 0)
            t.column(fame, defaultValue: 0)
            t.column(rename, defaultValue: 0)
            t.column(delete_date, defaultValue: 0)
            t.column(moves, defaultValue: 0)
            t.column(unban_time, defaultValue: 0)
            t.column(font, defaultValue: 0)
            t.column(uniqueitem_counter, defaultValue: 0)
            t.column(sex)
            t.column(hotkey_rowshift, defaultValue: 0)
            t.column(hotkey_rowshift2, defaultValue: 0)
            t.column(clan_id, defaultValue: 0)
            t.column(last_login)
            t.column(title_id, defaultValue: 0)
            t.column(show_equip, defaultValue: 0)
        })

        try db.run(char.createIndex(name, unique: true, ifNotExists: true))
        try db.run(char.createIndex(account_id, ifNotExists: true))
        try db.run(char.createIndex(party_id, ifNotExists: true))
        try db.run(char.createIndex(guild_id, ifNotExists: true))
        try db.run(char.createIndex(online, ifNotExists: true))

        try db.execute("UPDATE SQLITE_SEQUENCE SET seq = 149999 WHERE name = 'char'")
    }
}
